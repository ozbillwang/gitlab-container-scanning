...
RSpec.shared_examples 'as container scanner' do |*unsupported_scanners|
  ...
  specify do
    expect(report['remediations']).to be_present unless unsupported_scanners.include?(Gcs::Environment.scanner)
  ...
  specify do
    expect(report['vulnerabilities']).to be_present unless unsupported_scanners.include?(Gcs::Environment.scanner)
